...
/**
 * GET /applications/check-exists?propertyId=123
 * Checks if the logged-in tenant already has a non-denied application for the given property.
 */
export const checkExistingApplication = async (
  ...
  try {
    const tenantCognitoId = req.user?.id; // From authMiddleware
    const propertyId = Number(req.query.propertyId);

    if (!tenantCognitoId || !propertyId) {
      res.status(400).json({ message: "Missing tenant or property ID" });
      return;
    }
    ...
    const existingApplication = await prisma.application.findFirst({
      where: {
        ...
        propertyId,
        status: {
          not: "Denied", // If it's Pending or Approved
        },
      },
    });
    ...
    res
      .status(500)
      .json({ message: "Internal server error while checking application" });
  }
};
